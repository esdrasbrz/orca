...
XboxController::XboxController(const String& targetMac)
    : _core(targetMac),
      ...
      _lx(0.0f), _ly(0.0f), _rx(0.0f), _ry(0.0f), _lt(0.0f), _rt(0.0f),
      _btnA(false), _btnB(false), _btnX(false), _btnY(false),
      _btnLB(false), _btnRB(false), _btnStart(false), _btnSelect(false),
      _dpadUp(false), _dpadDown(false), _dpadLeft(false), _dpadRight(false) {}
...
float XboxController::leftX() const { return _lx; }
float XboxController::leftY() const { return _ly; }
float XboxController::rightX() const { return _rx; }
float XboxController::rightY() const { return _ry; }
float XboxController::leftTrigger() const { return _lt; }
float XboxController::rightTrigger() const { return _rt; }

bool XboxController::a() const { return _btnA; }
bool XboxController::b() const { return _btnB; }
bool XboxController::x() const { return _btnX; }
bool XboxController::y() const { return _btnY; }
bool XboxController::lb() const { return _btnLB; }
bool XboxController::rb() const { return _btnRB; }
bool XboxController::start() const { return _btnStart; }
bool XboxController::select() const { return _btnSelect; }
bool XboxController::dpadUp() const { return _dpadUp; }
bool XboxController::dpadDown() const { return _dpadDown; }
bool XboxController::dpadLeft() const { return _dpadLeft; }
bool XboxController::dpadRight() const { return _dpadRight; }

void XboxController::printTelemetry() const {
  char btnBuf[32] = "";
  if (_btnA) strcat(btnBuf, "A ");
  if (_btnB) strcat(btnBuf, "B ");
  if (_btnX) strcat(btnBuf, "X ");
  if (_btnY) strcat(btnBuf, "Y ");
  if (_btnLB) strcat(btnBuf, "LB ");
  if (_btnRB) strcat(btnBuf, "RB ");
  if (_btnStart) strcat(btnBuf, "START ");
  if (_btnSelect) strcat(btnBuf, "SELECT ");
  if (strlen(btnBuf) > 0 && btnBuf[strlen(btnBuf) - 1] == ' ') {
    btnBuf[strlen(btnBuf) - 1] = '\0';
  }

  char dpadBuf[24] = "";
  if (_dpadUp) strcat(dpadBuf, "UP ");
  if (_dpadDown) strcat(dpadBuf, "DOWN ");
  if (_dpadLeft) strcat(dpadBuf, "LEFT ");
  if (_dpadRight) strcat(dpadBuf, "RIGHT ");
...
float XboxController::normalizeAxis(uint16_t raw, bool invert) {
  ...
  if (norm < -1.0f) norm = -1.0f;
  if (fabs(norm) < DEADZONE) norm = 0.0f;
  return norm;
}
